import { diseaseLineChartData } from '../../data/mock/chartData';

export default function DiseaseLineChart() {
  const maxValue = Math.max(...diseaseLineChartData.map((d) => d.count));
  const width = 500;
  const height = 300;
  const padding = 40;
  ...
  const points = diseaseLineChartData.map((d, i) => {
    const x = padding + (i / (diseaseLineChartData.length - 1)) * chartWidth;
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
      <h2 className="text-lg font-bold mb-4">Disease Cases Trend</h2>
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="max-w-full">
        ...
        {points.map((p, i) => (
          <text key={`label-${i}`} x={p.x} y={height - padding + 20} textAnchor="middle" fontSize="12" fill="#6b7280">
            {p.date}
          </text>
        ))}
